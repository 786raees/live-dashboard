...
export default class MinorProcedure_datewisel2 extends Component{
  constructor(props){
    super(props);
    this.state = {
      chartData: {        
    ...
    this.MajorProcL3 = this.MajorProcL3.bind(this);  
   
  }

  componentDidMount(){
    LiveDashboardServices.getdatewiseMinProcL3(this.props.location.state.date_name,this.props.location.state.month_name).then((res) => {      
      var labels = [];
  ...
  getElementAtEvent = element => {
    ...
    }, () => this.MajorProcL3(this.state.clickedElement,this.props.location.state.date_name,this.state.speciality_name) );
    
  }

  
  MajorProcL3(date_name, speciality_name){

    this.props.history.push({
      pathname: '/MajorProcL3',
      state: {date_name: date_name, speciality_name: speciality_name}
    })
  }
  ...
  render(){    
    ...
    return (      
      ...
      <div className="App">
      <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Speciality wise Minor Procedures for the selected date <p style={{ color: 'yellow' }}> {this.props.location.state.date_name+"-"+ this.props.location.state.month_name}</p></h2>
