...
export default class OPD_datewiseCountsL2 extends Component {
  constructor(props) {
    ...
    this.ViewOPD4 = this.ViewOPD4.bind(this);

  }

  componentDidMount() {
    LiveDashboardServices.getdatewiseOPDCountsL3(this.props.location.state.date_name,this.props.location.state.month_name).then((res) => {
      var labels = [];
  ...
  getElementAtEvent = element => {
    ...
    }, () => this.ViewOPD4(this.state.clickedElement, this.props.location.state.date_name,this.props.location.state.month_name, this.state.speciality_name));

  }


  ViewOPD4(date_name, month_name, speciality_name) {
  ...
  render() {
    ...
    return (
      <Card className="card" border="info" style={{ marginLeft: '100px', marginRight: '100px', marginBottom: '100px' }}>
        <div className="App">
          <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Speciality wise OPD Visits for the selected date = <p style={{ color: 'yellow' }}> {this.props.location.state.date_name+"-"+ this.props.location.state.month_name}</p></h2>
