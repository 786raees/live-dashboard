...
export default class MajorProcedure_datewisel2 extends Component{
  constructor(props){
    super(props);
    this.state = {
      chartData: {        
  ...
  componentDidMount(){
    LiveDashboardServices.getdatewiseMajProcL3(this.props.location.state.date_name,this.props.location.state.month_name).then((res) => {      
      var labels = [];
  ...
  render(){    
    ...
    return (      
      ...
      <div className="App">
      <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Speciality wise Major Procedures for the selected date <p style={{ color: 'yellow' }}> {this.props.location.state.date_name+"-"+ this.props.location.state.month_name}</p></h2>
