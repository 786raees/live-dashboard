...
export default class Pathology_monthwiseL1 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      chartData: {
    ...
    this.ViewLabL2 = this.ViewLabL2.bind(this);
  }

  componentDidMount() {
    LiveDashboardServices.getmonthwiseLabL1().then((res) => {
      var labels = [];
  ...
  getElementAtEvent = element => {
    ...
    }, () => this.ViewLabL2(this.state.clickedElement, this.state.month_name));
  }


  ViewLabL2(month_name) {
  ...
  render() {
    ...
    return (
      <Card className="card" border="info" style={{ marginLeft: '180px', marginRight: '100px', marginBottom: '100px' }}>
        <div className="App">
          <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Month wise Pathology Tests for the defined period</h2>
