...
export default class Revenue_datewiseL2 extends Component {
  ...
  componentDidMount() {LiveDashboardServices.getdatewiseRevenueL2(this.props.location.state.month_name).then((res) => {
      var labels = [];
  ...
  render() {
    ...
    return (
      <Card className="card" border="info" style={{ marginLeft: '100px', marginRight: '100px', marginBottom: '100px' }}>
        <div className="App">
          <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Date wise Revenue in Million for the Month of <p style={{ color: 'yellow' }}> {this.props.location.state.month_name} </p> </h2>
          <Bar height="120px"
