...
export default class OPD_datewiseCountsL2 extends Component{
  constructor(props){
    ...
    this.ViewIPD4 = this.ViewIPD4.bind(this);  
   
  }

  componentDidMount(){
    LiveDashboardServices.getdatewiseIPDCountsL3(this.props.location.state.date_name,this.props.location.state.month_name).then((res) => {      
      var labels = [];
  ...
  getElementAtEvent = element => {
    ...
    }, () => this.ViewIPD4(this.state.clickedElement, this.props.location.state.date_name,this.props.location.state.month_name, this.state.speciality_name));

  }


  ViewIPD4(date_name, month_name, speciality_name) {
  ...
  render(){    
    const options = {
     /* title:{
        display:true,
        text:"Speciality wise No of IPD Admissions for the date=" + this.props.location.state.date_name,
        textAlign: 'center',
        background: 'blue',
        fontColor: "red",
        fontSize: 18
    ...
    return (      
      ...
      <div className="App">
      <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Speciality wise IPD Admissions for the selected date <p style={{ color: 'yellow' }}> {this.props.location.state.date_name+"-"+ this.props.location.state.month_name}</p></h2>
