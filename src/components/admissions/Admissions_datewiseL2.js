...
export default class Admissions_datewiseL1 extends Component{
  constructor(props){
    ...
    this.ViewIPD3 = this.ViewIPD3.bind(this);    
  }

  componentDidMount(){
    LiveDashboardServices.getdatewiseIPDCountsL2(this.props.location.state.month_name).then((res) => {      
      var labels = [];
  ...
  getElementAtEvent = element => {
    ...
    }, () => this.ViewIPD3(this.state.clickedElement, this.props.location.state.month_name, this.props.location.state.date_name));
  }


  ViewIPD3(date_name,month_name) {
  ...
  render(){    
    ...
    return (      
      ...
      <div className="App">
      <h2 style={{ color: 'white', textAlign: 'center', background: 'blue' }}>Date wise IPD Admissions for the Month of <p style={{ color: 'yellow' }}> {this.props.location.state.month_name} </p> </h2>
         <Bar height="120px"
